...
const CoverLetterPreview: React.FC<CoverLetterPreviewProps> = ({ data, templateId, onEdit, onDownload, isThumbnail = false }) => {
  if (!data) return null;
  ...
  return (
    <div className={isThumbnail ? "" : "flex gap-8"}>
      ...
      <div className={isThumbnail ? "" : "flex-1"}>
        <div className={`bg-white rounded-xl shadow-sm ${isThumbnail ? 'p-4' : 'p-8'}`} id={isThumbnail ? undefined : "cover-letter-preview"}>
          {/* Sender Information */}
          <div className="mb-8">
